...
#include <inttypes.h>
extern node *d_p0; //tips + 3*inter
extern node **d_nodep; // 2*tr->mxtips
extern node **h_nodep; // 2*tr->mxtips
extern node *buffList; //tips + 3*inter
extern node **listAddr; //tips + 3*inter
extern node **listAddr2d; //tips + 3*inter

extern int timesCalledNewz;
...
extern double * h_dlnLdlz;
extern double * h_d2lnLdlz2;
extern double * d_dlnLdlz;
extern double * d_d2lnLdlz2;
...
void kernelNewzCore(int , int, int, tree *);
node * findDevPointer(tree *, node *);
void transferList(tree *);
void transferUpdateList(tree *);
...
void newzCoreGPU(int firstIteration, tree *tr)
{
...
#endif
    //exec kernel
    kernelNewzCore(firstIteration, pIsTip, qIsTip, tr);
...
#ifdef out
    //transfer back dlnLdlz, d2lnLdlz2
    //need to transfer back xVectors??
    ...
    cudaMemcpy(h_dlnLdlz, d_dlnLdlz, alignLength*sizeof(double), cudaMemcpyDeviceToHost);
        error = cudaGetLastError();
        if(error != cudaSuccess)
        {
            // something's gone wrong
            // print out the CUDA error as a string
            printf("CUDA Error AFTER cudaMemcpy h_dlnLdlz: %s\n", cudaGetErrorString(error));

            // we can't recover from the error -- exit the program
            exit(1);
        }
    cudaMemcpy(h_d2lnLdlz2, d_d2lnLdlz2, alignLength*sizeof(double), cudaMemcpyDeviceToHost);
        error = cudaGetLastError();
        if(error != cudaSuccess)
        {
            // something's gone wrong
            // print out the CUDA error as a string
            printf("CUDA Error AFTER cudaMemcpy h_d2lnLdlz2: %s\n", cudaGetErrorString(error));
...
static void topLevelMakenewz(tree *tr, double *z0, int _maxiter, double *result)
{
    ...
    assert(0);
#ifndef use_pathNewzcoreGPU
/*
 * transfer list START
 */
    int total = tr->mxtips + 3*(tr->mxtips - 1);
    int i, j;
    nodeptr lptr;
    cudaError_t error;
    ...
    for(i=0; i<total; i++)
    {
        ...
        /* first put data */
        buffList[i].x = lptr->x;
        buffList[i].number = lptr->number;
        buffList[i].support = lptr->support; //dont need
        buffList[i].hash = lptr->hash; //dont need
        buffList[i].z[0] = lptr->z[0]; //TODO: numbranches == 1??
        

        for(j=0; j<total; j++)
        {
            if (listAddr[j] == lptr->next)
                buffList[i].next = listAddr2d[j];
            if (listAddr[j] == lptr->back)
                buffList[i].back = listAddr2d[j];
        }
        if (lptr->back == NULL)
            buffList[i].back = NULL;
        if (lptr->next == NULL)
            buffList[i].next = NULL;
    }
    ...
    {         
        for(j=0; j<total; j++)           
        {              
            if (listAddr[j] == tr->nodep[i])                  
                h_nodep[i] = listAddr2d[j];           
        }
    }
    
    
    cudaMemcpy(d_p0, buffList, total*sizeof(node), cudaMemcpyHostToDevice);
    error = cudaGetLastError();        
    if(error != cudaSuccess)        
    {            
        // something's gone wrong            
        // print out the CUDA error as a string
        printf("CUDA Error AFTER cudaMemcpy d_p0: %s\n", cudaGetErrorString(error));
        // we can't recover from the error -- exit the program            
        exit(1);
        
    }

    cudaMemcpy(d_nodep, h_nodep, 2*tr->mxtips*sizeof(node *), cudaMemcpyHostToDevice);
    error = cudaGetLastError();        
    if(error != cudaSuccess)        
    {            
        // something's gone wrong            
        // print out the CUDA error as a string
        printf("CUDA Error AFTER cudaMemcpy d_nodep: %s\n", cudaGetErrorString(error));
        // we can't recover from the error -- exit the program            
        exit(1);
 ...
 */    
  ...
  if(tr->executeModel[0])
  {
    ...
    pathNewzcoreGPU(pIsTip, qIsTip, tr, z0[0], _maxiter);
    
    
    //transfer results back
    
    cudaMemcpy(result, d_dlnLdlz, sizeof(double), cudaMemcpyDeviceToHost);
    //z[0] = result[0];    
    error = cudaGetLastError();        
    if(error != cudaSuccess)        
    {            
        // something's gone wrong            
        // print out the CUDA error as a string
        printf("CUDA Error AFTER cudaMemcpy result: %s\n", cudaGetErrorString(error));
        // we can't recover from the error -- exit the program            
        exit(1);
    ...
    tr->executeModel[0] = TRUE;
    //to z[0] to evala sto d_dlnLdlz
    //result[0] = z[0];
  ...
  do
      ...
      else
	{
      ...
      {
        dlnLdlz[0]   += h_dlnLdlz[i];
        d2lnLdlz2[0] += h_d2lnLdlz2[i];
      }      
  ...
  do
      ...
      else
	{
      ...
      printf("host reduction dlnLdlz: %f\n", dlnLdlz[0]);
      printf("host reduction d2lnLdlz2: %f\n", d2lnLdlz2[0]);
...
void makenewzGeneric(tree *tr, nodeptr p, nodeptr q, double *z0, int maxiter, double *result, boolean mask)
{
    ...
    /* START find list pointers for device*/
    nodeptr d_p, d_q;
    d_p = findDevPointer(tr, p);
    d_q = findDevPointer(tr, q);
  ...
  if(tr->executeModel[0])
  {
        ...
        /*if (transTree)
        {
            cudaMemcpy(d_tree, tr, (size_t)sizeof(tree), cudaMemcpyHostToDevice);
            error = cudaGetLastError();
            if(error != cudaSuccess)
            {
                // something's gone wrong
    ...
    //START execute on gpu
    assert(tr->numBranches==1);
    assert(mask==0);
    pathNewzcoreGPU(pIsTip, qIsTip, tr, z0[0], maxiter, d_p, d_q);
    
    
    //START transfer results back
    cudaMemcpy(result, d_dlnLdlz, sizeof(double), cudaMemcpyDeviceToHost);
    error = cudaGetLastError();        
    if(error != cudaSuccess)        
    {            
    ...
    /*
     * START transfer/update list back
     */ 
    transferUpdateList(tr);
