...
extern void *globalpStart;
extern void *d_startMisaligned;
extern int *testData;
...
#ifdef _WAYNE_MPI
  bootstrapsPerformed = n * processes;
#else
	...
	if(bootStopIt)
	  {
	    switch(tr->bootStopCriterion)
	      {
	      case FREQUENCY_STOP:
		printBothOpen("Stopped Rapid BS search after %d replicates with FC Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("Pearson Average of %d random splits: %f\n",BOOTSTOP_PERMUTATIONS , pearsonAverage);	      
		break;
	      case MR_STOP:
		printBothOpen("Stopped Rapid BS search after %d replicates with MR-based Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);	     
		break;
	      case MRE_STOP:
		printBothOpen("Stopped Rapid BS search after %d replicates with MRE-based Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);	     
		break;
	      case MRE_IGN_STOP:
		printBothOpen("Stopped Rapid BS search after %d replicates with MRE_IGN-based Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);	     
		break;
	      default:
		assert(0);
	  ...
	  { 
	    switch(tr->bootStopCriterion)	     
	      {
	      case FREQUENCY_STOP:
		printBothOpen("Rapid BS search did not converge after %d replicates with FC Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("Pearson Average of %d random splits: %f\n",BOOTSTOP_PERMUTATIONS , pearsonAverage);
		break;
	      case MR_STOP:
		printBothOpen("Rapid BS search did not converge after %d replicates with MR-based Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);
		break;
	      case MRE_STOP:
		printBothOpen("Rapid BS search did not converge after %d replicates with MRE-based Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);
		break;
	      case MRE_IGN_STOP:
		printBothOpen("Rapid BS search did not converge after %d replicates with MR_IGN-based Bootstopping criterion\n", bootstrapsPerformed);
		printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);
		break;
	      default:
		assert(0);
  ...
  if(adef->bootStopping)
      ...
      if(bootStopIt)
	{
	  switch(tr->bootStopCriterion)
	    {
	    case FREQUENCY_STOP:
	      printBothOpen("Stopped Standard BS search after %d replicates with FC Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("Pearson Average of %d random splits: %f\n",BOOTSTOP_PERMUTATIONS , pearsonAverage);	      
	      break;
	    case MR_STOP:
	      printBothOpen("Stopped Standard BS search after %d replicates with MR-based Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);	     
	      break;
	    case MRE_STOP:
	      printBothOpen("Stopped Standard BS search after %d replicates with MRE-based Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);	     
	      break;
	    case MRE_IGN_STOP:
	      printBothOpen("Stopped Standard BS search after %d replicates with MRE_IGN-based Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);	     
	      break;
	    default:
	      assert(0);
      ...
      else
	{
	  switch(tr->bootStopCriterion)
	    {
	    case FREQUENCY_STOP:
	      printBothOpen("Standard BS search did not converge after %d replicates with FC Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("Pearson Average of %d random splits: %f\n",BOOTSTOP_PERMUTATIONS , pearsonAverage);
	      break;
	    case MR_STOP:
	      printBothOpen("Standard BS search did not converge after %d replicates with MR-based Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);
	      break;
	    case MRE_STOP:
	      printBothOpen("Standard BS search did not converge after %d replicates with MRE-based Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);
	      break;
	    case MRE_IGN_STOP:
	      printBothOpen("Standard BS search did not converge after %d replicates with MR_IGN-based Bootstopping criterion\n", bootstrapsPerformed);
	      printBothOpen("WRF Average of %d random splits: %f\n", BOOTSTOP_PERMUTATIONS, pearsonAverage);
	      break;
	    default:
	      assert(0);
...
extern float T2Dnewz, KenrelTnewz, T2Hnewz;
extern float oneKernelTnewz;
void doInference(tree *tr, analdef *adef, rawdata *rdta, cruncheddata *cdta)
{
  ...
  cudaFree(d_startMisaligned);
        cudaError_t error = cudaGetLastError();
	if(error != cudaSuccess)
	{
	    // something's gone wrong
	    // print out the CUDA error as a string
	    printf("CUDA Error AFTER cudaFree: %s\n", cudaGetErrorString(error));
		
	    // we can't recover from the error -- exit the program
	    exit(1);
	}
        
  //free(startMisaligned);
...
#ifndef MEMORG_TIMES
  printf ("----------------\n");
  printf ("times called newView: %d\n", timesCalledNewView);
  printf ("Device newView trans: %f ms\n", T2DnewView);
  printf ("Host newView trans: %f ms\n", T2HnewView);
  printf ("Kenrel newView: %f ms\n", KenrelTnewView);
  printf ("sum Host Dev newView trans: %f ms\n", T2DnewView + T2HnewView);
  printf ("Total kernel+trans newView: %f ms\n", T2DnewView + T2HnewView + KenrelTnewView);
  
  
  printf ("----------------\n");
  printf ("times called evaluate: %d\n", timesCalledEvaluate);
  printf ("Device evaluate trans: %f ms\n", T2Devaluate);
  printf ("Host evaluate trans: %f ms\n", T2Hevaluate);
  printf ("Kenrel evaluate: %f ms\n", KenrelTevaluate);
  printf ("sum Host Dev evaluate trans: %f ms\n", T2Devaluate + T2Hevaluate);
  printf ("Total kernel+trans evaluate: %f ms\n", T2Devaluate + T2Hevaluate + KenrelTevaluate);
  
  printf ("----------------\n");
  printf ("times called trans: %d\n", timesCalledNewz);
  printf ("Device newz trans: %f ms\n", T2Dnewz);
  printf ("Host newz trans: %f ms\n", T2Hnewz);
  printf ("Kenrel newz: %f ms\n", KenrelTnewz);
  printf ("oneKenrel newz: %f ms\n", oneKernelTnewz); 
  printf ("sum Host Dev newz trans: %f ms\n", T2Dnewz + T2Hnewz);
  printf ("Total kernel+trans newz: %f ms\n", T2Dnewz + T2Hnewz + KenrelTnewz);  
  
//  printf("\n!!! full time kernel: %f\n", fullTimeKe); 
//  printf("\n!!! full time transfers: %f\n", (double)fullTimeTr/(double)2927000000);
