...
void bigPathTreeEval(double, node *);
void bigPathRearrange(node *, int, int );

void transferUpdateList(tree *);
void transferList(tree *);
node *findDevPointer(tree *, node *);
node *findHostPointer(tree *, node *);
    ...
    else
    {
        cudaMemcpy(d_tree, tr, (size_t)sizeof(tree), cudaMemcpyHostToDevice);
            ...
            if(error != cudaSuccess)
            {
                // something's gone wrong
                // print out the CUDA error as a string
                printf("CUDA Error AFTER cudaMemcpy transferTR: %s\n", cudaGetErrorString(error));
...
void transferTRback(tree *tr)
{
    cudaError_t error;

    cudaMemcpy(tr, d_tree, (size_t)sizeof(tree), cudaMemcpyDeviceToHost);
            error = cudaGetLastError();
            if(error != cudaSuccess)
            {
                // something's gone wrong
...
{  
#ifdef MEMORG
    nodeptr d_p; //starting pointer in the device list
    nodeptr tmpNode;
    ...
    bigPathRearrange(d_p, mintrav, maxtrav);
    
    transferUpdateList(tr);
    transferTRback(tr);
    
    tmpNode = findHostPointer(tr, tr->insertNode);
    if(tmpNode!=NULL)
        tr->insertNode = tmpNode;
    
    tmpNode = findHostPointer(tr, tr->removeNode);
    if(tmpNode!=NULL)
        tr->removeNode = tmpNode;
...
boolean treeEvaluate (tree *tr, double smoothFactor)       /* Evaluate a user tree */
{
    ...
    node *d_tr_start;
    d_tr_start = findDevPointer(tr, tr->start);
    
    transferTR(tr);
    transferList(tr);

    bigPathTreeEval(smoothFactor, d_tr_start);
    
    transferUpdateList(tr);
    transferTRback(tr);
    //tr->insertNode = findHostPointer(tr, tr->insertNode);
    //tr->removeNode = findHostPointer(tr, tr->removeNode);
    tr->start = findHostPointer(tr ,tr->start); 
